...
class Bcc(http.Controller):
    ...
    def index(self, **kw):
        log.info("Handling compliance license status")
        self._auth(http.request.params)

        payload = http.request.params["data"]

        log.debug("Storing license data for {}".format(payload))
        ...
        try:
            ...
            raise exceptions.ValidationError("Unable to handle License updates")
    ...
    def partner_metadata(self, partner_id, token=None):
        log.info("Handling CA license status")
        self._auth(http.request.params)
        ...
        for lic_rec in partner.bcc_license_data:
            _res["licenses"].append({
                "number": lic_rec.license_number,
                "status": lic_rec.status,
                "is_valid": lic_rec.validate_license_status(alert=False),
                "issue_date": lic_rec.issue_date,
                "expiration_date": lic_rec.expiration_date,
    ...
    def _auth(self, body):
        try:
            log.info("Validating scrapper API access token")
